...
echo "Replace current edition $LAST_EDITION with $PREVIOUS_EDITION"  
sql -S ${DB_USER}/${DB_PASSWORD}@${TNS_SERVICE}  <<-EOF
SET ECHO OFF
-- Exit on error
WHENEVER SQLERROR EXIT SQL.SQLCODE ROLLBACK;
...
if [ ${COUNTLASTTAG} -eq 0 ]
	then
		...
		-- BUG Workaround: Clean ACTION table with orphan records
		COMMIT;
		DELETE FROM DATABASECHANGELOG_ACTIONS WHERE DEPLOYMENT_ID NOT IN (SELECT DEPLOYMENT_ID FROM DATABASECHANGELOG);
		COMMIT;
		QUIT
EOF
fi 
